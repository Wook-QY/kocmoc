...
local hometab = Window:CreateTab("首页")
...
convertatslider = farmo:CreateSlider("将在", 0, 100, 100, false, function(Value) kocmoc.vars.convertat = Value end)
...
contt:CreateToggle("不要转化花粉", nil, function(State) kocmoc.toggles.disableconversion = State end)
contt:CreateToggle("自动减袋",nil,function(Boole) kocmoc.toggles.autouseconvertors = Boole end)
contt:CreateDropdown("减袋模式",{"Ticket Converters","Just Snowflakes","Just Coconuts","Snowflakes and Coconuts","Tickets and Snowflakes","Tickets and Coconuts","All"},function(Select) kocmoc.vars.autouseMode = Select end)
contt:CreateSlider("降价确认时间",3,20,10,false,function(tttttttt) kocmoc.vars.autoconvertWaitTime = tonumber(tttttttt) end)
...
farmo:CreateToggle("农场椰子 & 淋浴", nil, function(State) kocmoc.toggles.farmcoco = State end)
farmo:CreateToggle("农场精确准星", nil, function(State) kocmoc.toggles.collectcrosshairs = State end)
farmo:CreateToggle("农场毛绒炸弹", nil, function(State) kocmoc.toggles.farmfuzzy = State end)
...
farmt:CreateToggle("自动种植园主", nil, function(State) kocmoc.toggles.autoplanters = State end):AddToolTip("Will re-plant your planters after converting, if they hit 100%")
...
farmt:CreateToggle("自动免票", nil, function(State) kocmoc.toggles.freeantpass = State end)
farmt:CreateToggle("农场豆芽", nil, function(State) kocmoc.toggles.farmsprouts = State end)
farmt:CreateToggle("农场泡芙", nil, function(State) kocmoc.toggles.farmpuffshrooms = State end)
-- BEESMAS MARKER farmt:CreateToggle("Farm Snowflakes [⚠️]", nil, function(State) kocmoc.toggles.farmsnowflakes = State end)
farmt:CreateToggle("传送到稀有物品 [⚠️]", nil, function(State) kocmoc.toggles.farmrares = State end)
farmt:CreateToggle("自动接受/确认任务 [⚙]", nil, function(State) kocmoc.toggles.autoquest = State end)
farmt:CreateToggle("自动完成任务 [⚙]", nil, function(State) kocmoc.toggles.autodoquest = State end)
farmt:CreateToggle("自动蜂蜜风暴", nil, function(State) kocmoc.toggles.honeystorm = State end)
farmt:CreateLabel(" ")
farmt:CreateToggle("在X次转换后重置蜜蜂能量",nil,function(bool) kocmoc.vars.resetbeeenergy = bool end)
farmt:CreateTextBox("量转换", "default = 3", true, function(Value) kocmoc.vars.resettimer = tonumber(Value) end)
